...
const { postToWeb } = require("@utils/httpUtils");
...
async function closeTicket(channel, closedBy, reason) {
  ...
  try {
    ...
    const logsUrl = await postToWeb(content, `Ticket Logs for ${channel.name}`);
...
async function openTicket(guild, user, config) {
  ...
  try {
    const applicationOwner = user.username;
    ...
    const tktChannel = await guild.channels.create(`tіcket-${user.username}`, {
      type: "GUILD_TEXT",
    ...
    const embed = new MessageEmbed()
      .setAuthor({ name: `Ticket #${applicationOwner}` })
