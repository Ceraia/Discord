...
async function performAutomod(message, settings) {
  ...
  if (automod.anti_spam) {
    const key = message.author.id + "|" + message.guildId;
    if (message.client.antiSpamCache.has(key)) {
      let antiSpamInfo = message.client.antiSpamCache.get(key);
      if (
        antiSpamInfo.channelId !== message.channelId &&
        antiSpamInfo.content === content &&
        Date.now() - antiSpamInfo.timestamp < 2000
      ) {
      ...
      message.client.antiSpamCache.set(key, antiSpamInfo);
    }
  }
